...
def save_occ(pred_c, pred_f, img_metas, path, gt_occ=None, free_id=0, thres_low=0.4, thres_high=0.99):
# def save_occ(pred_c, pred_f, img_metas, path, visible_mask=None, gt_occ=None, free_id=0, thres_low=0.4, thres_high=0.99):
    ...
    4. pred_f visible
    """
    pred_f = F.softmax(pred_f, dim=1)  # (1, 17, 512, 512, 40)
    pred_f = pred_f[0].cpu().numpy()  # C W H D    # (17, 512, 512, 40)

    pred_c = F.softmax(pred_c, dim=1)  # (1, 17, 128, 128, 10)
    pred_c = pred_c[0].cpu().numpy()  # C W H D  # (17, 128, 128, 10)

    # visible_mask = visible_mask[0].cpu().numpy().reshape(-1) > 0  # WHD
    # gt_occ = gt_occ.data[0][0].cpu().numpy()  # W H D

    # 调试gt_occ
    # print(f"gt_occ type: {type(gt_occ)}")
    # print(f"gt_occ value: {gt_occ}")
    # if gt_occ is None:
    #     print("gt_occ is None!")
    # print(f"Inside get_gt_occ: gt_occ type: {type(gt_occ)}")
    # print(f"Inside get_gt_occ: gt_occ shape: {gt_occ.data[0].shape}")  # gt_occ shape: torch.Size([1, 512, 512, 40])
    gt_occ = gt_occ.data[0][0].cpu().numpy()  # W H D
